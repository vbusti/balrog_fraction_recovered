...
for tile in tiles:
    ...
    for band in bands:
        ...
        if not os.path.isdir('../results/'+tile):
            os.mkdir('../results/'+tile)

        if not os.path.isdir('../results/'+tile+'/'+band):
            os.mkdir('../results/'+tile+'/'+band)


        # data plus injections

        cat_inj = pf.open("/home/vinicius/Documents/balrog/fraction_recovered/data/"+tile+"/"+tile+"_"+band+"_cat.fits")
        cat_inj.info()
        ...
        cat_true = pf.open("/home/vinicius/Documents/balrog/fraction_recovered/data/"+tile+"/"+tile+"_0_balrog_truth_cat.fits")
        cat_true.info()
        ...
        plt.savefig('../results/'+tile+'/'+band+'/mag_'+band+'_dist_injected.png')
        plt.close()
        ...
        for i in range(len(ra_t)):
            mini_mask = (np.abs(ra_i-ra_t[i]) < 0.2)*(np.abs(dec_i-dec_t[i])<0.2)
        ...
        plt.savefig('../results/'+tile+'/'+band+'/mag_'+band+'_vs_fraction.png')
        plt.close()

        np.savetxt('../results/'+tile+'/'+band+'/mag_fraction.txt',np.array([mean_mag_bin,frac_bin]).T)
        ...
        plt.ylim(-2,2)
        plt.savefig('../results/'+tile+'/'+band+'/mag_'+band+'_vs_deltamag.png')
        plt.close()

        np.savetxt('../results/'+tile+'/'+band+'/mag_deltamag.txt',np.array([mag_t_band,mag_t_band - mag_inj]).T)
        ...
        np.savetxt('../results/'+tile+'/'+band+'/mag_deltamag.txt',np.array([mean_dm,std_dm]))
